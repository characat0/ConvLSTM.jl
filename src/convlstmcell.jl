...
const _ConvLSTMInputType = Tuple{<:AbstractArray,Tuple{<:AbstractArray,<:AbstractArray}}
...
function ConvLSTMCell(
    ...
    return ConvLSTMCell(static(peephole), Wx, Wh, init_state, init_memory)
end

Lux.initialstates(rng::AbstractRNG, ::ConvLSTMCell) =
...
function (lstm::ConvLSTMCell)(x::AbstractArray, ps, st::NamedTuple)
...
function (lstm::ConvLSTMCell)(
    (x, (hiddenₙ, memoryₙ))::_ConvLSTMInputType,
    ps,
    st::NamedTuple,
)
...
function Base.show(io::IO, lstm::ConvLSTMCell)
    print(
        io,
        "ConvLSTMCell($(lstm.Wx.kernel_size), $(lstm.Wh.kernel_size), $(lstm.Wx.in_chs => lstm.Wx.out_chs)",
    )
